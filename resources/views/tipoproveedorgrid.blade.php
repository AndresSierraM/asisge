...
@section('titulo')<h3 class="pestana" id="titulo"><center>Tipo de Proveedor</h3>@stop
@section('content')
{!!Form::model($datos)!!}
<style>
    tfoot input {
        ...
        <div class="container">
            <div class="row">
                <div class="container">
                    ...
                    <table id="ttipoproveedor" name="ttipoproveedor" class="display table-bordered" width="100%">
                        <thead>
                            <tr class="btn-primary active">
                                <th style="width:60px;padding: 1px 8px;" data-orderable="false">
                                <a href="tipoproveedor/create"><span style= "display: <?php echo $visible;?> color:white;" class="glyphicon glyphicon-plus"></span></a>
...
<script type="text/javascript">
    ...
    $(document).ready( function () {
        ...
        /*$('#ttipoproveedor').DataTable({
            "aProcessing": true,
            "aServerSide": true,
            "stateSave":true,
            "ajax": "{!! URL::to ('/datosTipoProveedor')!!}",
        });*/
        var lastIdx = null;
        var modificar = '<?php echo (isset($datos[0]) ? $dato["modificarRolOpcion"] : 0);?>';
        var eliminar = '<?php echo (isset($datos[0]) ? $dato["eliminarRolOpcion"] : 0);?>';
        var table = $('#ttipoproveedor').DataTable( {
            "order": [[ 1, "asc" ]],
            "aProcessing": true,
            "aServerSide": true,
            "stateSave":true,
            "ajax": "{!! URL::to ('/datosTipoProveedor?modificar="+modificar+"&eliminar="+eliminar+"')!!}",
            "language": {
    ...
    $('#ttipoproveedor tfoot th').each( function () {
        if($(this).index()>0){
        var title = $('#ttipoproveedor thead th').eq( $(this).index() ).text();
