...
$grid->SelectCommand = 'SELECT idInspeccion, fechaElaboracionInspeccion, nombreTipoInspeccion, nombreCompletoTercero
                        FROM inspeccion I
                        left join tipoinspeccion TI
                        on I.TipoInspeccion_idTipoInspeccion = TI.idTipoInspeccion
                        left join tercero T
                        on I.Tercero_idRealizadaPor = T.idTercero';
...
$grid->setUrl('inspecciongrid');
// SetOpciones de la grid
$grid->setGridOptions(array(
    "rowNum"=>30,
    "rowList"=>array(30,50,100),
    "sortname"=>"fechaElaboracionInspeccion",
    "multiSort"=>true,
    "sortable"=>true,
    "altRows"=>true,
    "hoverrows"=>true,
...
$grid->setColProperty("idInspeccion", array(
...
$grid->setColProperty("fechaElaboracionInspeccion", array(
  "searchoptions"=>array("sopt"=>array("bw", "ne","le","lt","ge","gt")),
  "formatter"=>"date",
  "label"=>"Fecha"
    )
);
$grid->setColProperty("nombreTipoInspeccion", array(
	"searchoptions"=>array("sopt"=>array("bw", "eq","ne","le","lt","ge","gt")),
	"label"=>"Tipo de Inspecci&oacute;n"
    )
);

$grid->setColProperty("nombreCompletoTercero", array(
	"searchoptions"=>array("sopt"=>array("bw", "ne","le","lt","ge","gt")),
	"label"=>"Realizada Por"
    )
);
...
$buttonmodificar = array("#pager",
    array("caption"=>"Modificar", "title"=>"Modificar el registro", 
      "onClickButton"=>"js: function(){
        ...
        if(id) {
          window.location.href = 'inspeccion/'+id+'/edit';
        } else {
...
$buttoneliminar = array("#pager",
    array("caption"=>"Eliminar", "title"=>"Eliminar el registro", 
      "onClickButton"=>"js: function(){
        var id = $('#grid').jqGrid('getGridParam','selrow'), data={};
        if(id) {
          window.location.href = 'inspeccion/'+id+'/edit?accion=eliminar';
        } else {
...
$buttonimprimir = array("#pager",
    array("caption"=>"Imprimir", "title"=>"Imprimir el registro", 
      "onClickButton"=>"js: function(){
        var id = $('#grid').jqGrid('getGridParam','selrow'), data={};
        if(id) {
          window.open('inspeccion/'+id+'?accion=imprimir','Formato','width=5000,height=5000,scrollbars=yes, status=0, toolbar=0, location=0, menubar=0, directories=0');
