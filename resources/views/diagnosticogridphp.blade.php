...
$grid->SelectCommand = 'SELECT idDiagnostico, codigoDiagnostico, nombreDiagnostico, fechaElaboracionDiagnostico, AVG(resultadoDiagnosticoDetalle) as resultado 
                        FROM diagnostico D
...
$grid->setUrl('diagnosticogrid');
// SetOpciones de la grid
$grid->setGridOptions(array(
    "rowNum"=>30,
    "rowList"=>array(30,50,100),
    "sortname"=>"codigoDiagnostico",
    "multiSort"=>true,
    "sortable"=>true,
    "altRows"=>true,
    "hoverrows"=>true,
...
$grid->setColProperty("idDiagnostico", array(
...
$grid->setColProperty("codigoDiagnostico", array(
	"searchoptions"=>array("sopt"=>array("bw", "eq","ne","le","lt","ge","gt")),
	"label"=>"C&oacute;digo"
    )
);

$grid->setColProperty("nombreDiagnostico", array(
	"searchoptions"=>array("sopt"=>array("bw", "ne","le","lt","ge","gt")),
	"label"=>"Nombre"
    )
);

$grid->setColProperty("fechaElaboracionDiagnostico", array(
  "searchoptions"=>array("sopt"=>array("bw", "ne","le","lt","ge","gt")),
  "formatter"=>"date",
  "label"=>"Fecha"
    )
...
$grid->setColProperty("resultado", array(
  "searchoptions"=>array("sopt"=>array("bw", "ne","le","lt","ge","gt")),
  "formatter"=>"integer",
  "formatoptions"=>array("thousandsSeparator"=>","),
  "label"=>"Resultado"
...
$buttonadicionar = array("#pager",
    array("caption"=>"Adicionar", "title"=>"Adicionar un nuevo registro", "onClickButton"=>"js: function(){
        window.location.href = 'diagnostico/create'}"
    )
);
...
$buttonmodificar = array("#pager",
    array("caption"=>"Modificar", "title"=>"Modificar el registro", 
      "onClickButton"=>"js: function(){
        ...
        if(id) {
          window.location.href = 'diagnostico/'+id+'/edit';
        } else {
...
$buttoneliminar = array("#pager",
    array("caption"=>"Eliminar", "title"=>"Eliminar el registro", 
      "onClickButton"=>"js: function(){
        var id = $('#grid').jqGrid('getGridParam','selrow'), data={};
        if(id) {
          window.location.href = 'diagnostico/'+id+'/edit?accion=eliminar';
        } else {
