...
@section('titulo')<h3 id="titulo"><center>Encuestas</center></h3>@stop
@section('content')
<style>
    tfoot input {
                width: 100%;
...
<script type="text/javascript">
    ...
    $(document).ready( function () {
        ...
        var table = $('#tencuesta').DataTable( {
            "order": [[ 1, "asc" ]],
            "aProcessing": true,
            "aServerSide": true,
            "stateSave":true,
            "ajax": "{!! URL::to ('/datosEncuesta?modificar="+modificar+"&eliminar="+eliminar+"')!!}",
            "language": {
    ...
    $('#tencuesta tfoot th').each( function () {
        if($(this).index()>0){
        var title = $('#tencuesta thead th').eq( $(this).index() ).text();
