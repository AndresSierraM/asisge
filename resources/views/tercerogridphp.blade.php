...
$grid->SelectCommand = 'SELECT idTercero, nombreTipoIdentificacion, documentoTercero, nombreCompletoTercero, estadoTercero
                        FROM tercero T
                        LEFT JOIN tipoidentificacion TI
                        ON T.TipoIdentificacion_idTipoIdentificacion = TI.idTipoIdentificacion';
...
$grid->setUrl('tercerogrid');
// SetOpciones de la grid
$grid->setGridOptions(array(
    "rowNum"=>30,
    "rowList"=>array(30,50,100),
    "sortname"=>"nombreCompletoTercero",
    "multiSort"=>true,
    "sortable"=>true,
    "altRows"=>true,
    "hoverrows"=>true,
...
// ----------------------------------------------------
$grid->setColProperty("idTercero", array(
...
$grid->setColProperty("documentoTercero", array(
...
$grid->setColProperty("estadoTercero", array(
  "searchoptions"=>array("sopt"=>array("bw", "ne","le","lt","ge","gt")),
  "label"=>"Estado"
    )
);
...
$buttonadicionar = array("#pager",
    array("caption"=>"Adicionar", "title"=>"Adicionar un nuevo registro", "onClickButton"=>"js: function(){
        window.location.href = 'tercero/create'}"
    )
);
...
$buttonmodificar = array("#pager",
    array("caption"=>"Modificar", "title"=>"Modificar el registro", 
      "onClickButton"=>"js: function(){
        ...
        if(id) {
          window.location.href = 'tercero/'+id+'/edit';
        } else {
...
$buttoneliminar = array("#pager",
    array("caption"=>"Eliminar", "title"=>"Eliminar el registro", 
      "onClickButton"=>"js: function(){
        var id = $('#grid').jqGrid('getGridParam','selrow'), data={};
        if(id) {
          window.location.href = 'tercero/'+id+'/edit?accion=eliminar';
        } else {
