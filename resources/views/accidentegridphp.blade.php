...
$grid->SelectCommand = 'SELECT idAccidente, numeroAccidente, nombreAccidente, fechaOcurrenciaAccidente, 
                              clasificacionAccidente
                        FROM accidente A
                        left join tercero T
                        on A.Tercero_idCoordinador = T.idTercero';
...
$grid->setUrl('accidentegrid');
// SetOpciones de la grid
$grid->setGridOptions(array(
    "rowNum"=>30,
    "rowList"=>array(30,50,100),
    "sortname"=>"fechaOcurrenciaAccidente",
    "multiSort"=>true,
    "sortable"=>true,
    "altRows"=>true,
    "hoverrows"=>true,
...
$grid->setColProperty("idAccidente", array(
...
$grid->setColProperty("numeroAccidente", array(
  "searchoptions"=>array("sopt"=>array("bw", "ne","le","lt","ge","gt")),
  "label"=>"N&uacute;mero"
    )
);

$grid->setColProperty("nombreAccidente", array(
  "searchoptions"=>array("sopt"=>array("bw", "ne","le","lt","ge","gt")),
  "label"=>"Descripci&oacute;n"
    )
);

$grid->setColProperty("fechaOcurrenciaAccidente", array(
  "searchoptions"=>array("sopt"=>array("bw", "ne","le","lt","ge","gt")),
  "formatter"=>"date",
...
$grid->setColProperty("clasificacionAccidente", array(
...
$buttonadicionar = array("#pager",
    array("caption"=>"Adicionar", "title"=>"Adicionar un nuevo registro", "onClickButton"=>"js: function(){
        window.location.href = 'accidente/create'}"
    )
);
...
$buttonmodificar = array("#pager",
    array("caption"=>"Modificar", "title"=>"Modificar el registro", 
      "onClickButton"=>"js: function(){
        ...
        if(id) {
          window.location.href = 'accidente/'+id+'/edit';
        } else {
...
$buttoneliminar = array("#pager",
    array("caption"=>"Eliminar", "title"=>"Eliminar el registro", 
      "onClickButton"=>"js: function(){
        var id = $('#grid').jqGrid('getGridParam','selrow'), data={};
        if(id) {
          window.location.href = 'accidente/'+id+'/edit?accion=eliminar';
        } else {
