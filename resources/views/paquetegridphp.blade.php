...
$grid->SelectCommand = 'SELECT idPaquete, ordenPaquete, nombrePaquete, iconoPaquete FROM paquete';
...
$grid->setColProperty("idPaquete", array(
...
$grid->setColProperty("ordenPaquete", array(
...
$grid->setColProperty("iconoPaquete", array(
  "formatter"=>"js:formatImage", 
  "unformat"=>"js:unformatImage",
  "label"=>"Icono"
    )
);
...
$buttoneliminar = array("#pager",
    array("caption"=>"Eliminar", "title"=>"Eliminar el registro", 
      "onClickButton"=>"js: function(){
        var id = $('#grid').jqGrid('getGridParam','selrow'), data={};
        if(id) {
          window.location.href = 'paquete/'+id+'/edit?accion=eliminar';
        } else {
