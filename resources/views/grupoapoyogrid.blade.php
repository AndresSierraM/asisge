...
@section('titulo')<h3 id="titulo"><center>Grupos de Apoyo</center></h3>@stop
@section('content')
<style>
    tfoot input {
                width: 100%;
                        ...
                        <thead>
                            <tr class="btn-default active">
                                <th style="width:40px;padding: 1px 8px;" data-orderable="false">
                                 <a href="grupoapoyo/create"><span style= "display: <?php echo $visible;?> " class="glyphicon glyphicon-plus"></span></a>
...
<script type="text/javascript">
    ...
    $(document).ready( function () {
        ...
        /*$('#tgrupoapoyo').DataTable({
            "aProcessing": true,
            "aServerSide": true,
            "stateSave":true,
            "ajax": "{!! URL::to ('/datosGrupoApoyo')!!}",
        });*/
        var lastIdx = null;
        var modificar = '<?php echo (isset($datos[0]) ? $dato["modificarRolOpcion"] : 0);?>';
        var eliminar = '<?php echo (isset($datos[0]) ? $dato["eliminarRolOpcion"] : 0);?>';
        var table = $('#tgrupoapoyo').DataTable( {
            "order": [[ 1, "asc" ]],
            "aProcessing": true,
            "aServerSide": true,
            "stateSave":true,
            "ajax": "{!! URL::to ('/datosGrupoApoyo?modificar="+modificar+"&eliminar="+eliminar+"')!!}",
            "language": {
    ...
    $('#tgrupoapoyo tfoot th').each( function () {
        if($(this).index()>0){
        var title = $('#tgrupoapoyo thead th').eq( $(this).index() ).text();
