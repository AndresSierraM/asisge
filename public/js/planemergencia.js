var wrapper = document.getElementById("signature-padR"),
...
if(document.getElementById("image-padR"))
{
    var wrapper2 = document.getElementById("image-padR"),
...
function mostrarFirmaRealizadaPor(reg)
{   
    if(document.getElementById("signature-padR").style.display == "block" || 
        document.getElementById("signature-padR").style.display == "")
        document.getElementById("signature-padR").style.display = "none";
    else
        document.getElementById("signature-padR").style.display = "block";

    if(reg != 'undefined')
        document.getElementById("signature-regR").value = reg;
    else
        document.getElementById("signature-regR").value = '';

    resizeCanvas() ;
}


function mostrarImagenRealizadaPor(ruta)
{

    if(document.getElementById("image-padR").style.display == "none" || 
        document.getElementById("image-padR").style.display == "")
        document.getElementById("image-padR").style.display = "none";
    else
        document.getElementById("image-padR").style.display = "none";
...
signaturePadR = new signaturePadR(canvas);
...
saveButton.addEventListener("click", function (event) {
    if (signaturePadR.isEmpty()) {
        alert("Por Favor Registre Su Firma.");
    } else {
        //window.open(signaturePadR.toDataURL());
        reg = '';
        if(document.getElementById("signature-regR").value != 'undefined')
            reg = document.getElementById("signature-regR").value;
        

        document.getElementById("firmaRealizadaPor"+reg).src = signaturePadR.toDataURL() ;
        document.getElementById("firmabase64RealizadaPor"+reg).value = signaturePadR.toDataURL() ;
        mostrarFirmaRealizadaPor();
    }
});
